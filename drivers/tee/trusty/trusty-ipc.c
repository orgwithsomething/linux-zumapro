			   ...
			   struct tipc_msg_buf *mb)
{
	...
	if (vds->state == VDS_ONLINE) {
		/*
		 * Premapped: the descriptor address is the FF-A handle the secure
		 * world resolves, not a CPU/DMA address.  Per-device dma_ops (the
		 * vendor's buf_id hook) do not exist on this arch, so we hand the
		 * handle to the vring directly.
		 */
		sg_init_one(&sg, mb, mb->wpos);
		sg_dma_address(&sg) = mb->buf_id;
		sg_dma_len(&sg) = mb->wpos;
		err = virtqueue_add_outbuf_premapped(vds->txvq, &sg, 1, mb,
						     GFP_KERNEL);
...
int tipc_chan_connect(struct tipc_chan *chan, const char *name)
{
	...
	/* fixed-size port name, zero-padded and always NUL-terminated */
	strscpy_pad(body->name, name, sizeof(body->name));
...
drop_it:
	/* add the buffer back to the virtqueue (premapped: addr == FF-A handle) */
	sg_init_one(&sg, rxbuf, rxbuf->buf_sz);
	sg_dma_address(&sg) = rxbuf->buf_id;
	sg_dma_len(&sg) = rxbuf->buf_sz;
	err = virtqueue_add_inbuf_premapped(vds->rxvq, &sg, 1, rxbuf, NULL,
					    GFP_KERNEL);
	if (err < 0) {
...
static int tipc_virtio_probe(struct virtio_device *vdev)
{
	...
	/* copy dev name (zero-padded, always NUL-terminated) */
	strscpy_pad(vds->cdev_name, config.dev_name, sizeof(vds->cdev_name));
	...
	for (i = 0; i < virtqueue_get_vring_size(vds->rxvq); i++) {
		...
		sg_dma_address(&sg) = rxbuf->buf_id;
		sg_dma_len(&sg) = rxbuf->buf_sz;
		err = virtqueue_add_inbuf_premapped(vds->rxvq, &sg, 1, rxbuf,
						    NULL, GFP_KERNEL);
